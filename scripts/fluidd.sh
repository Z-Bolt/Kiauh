...
function get_fluidd_download_url() {
  ...
  latest_url="https://github.com/Z-Bolt/fluidd/releases/download/${latest_tag}/fluidd.zip"

  ### get stable fluidd download url
  stable_tag=$(echo "${tags}" | grep -E "^v([0-9]+\.?){3}$" | head -1)
  stable_url="https://github.com/Z-Bolt/fluidd/releases/download/${stable_tag}/fluidd.zip"
